...
export function ReviewStep({ form, classInfo, isSubmitting, onSubmit, submitLabel, appliedPromo, onApplyPromo }: Props) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-lg font-semibold text-stone-800">Review & Submit</h2>
      ...
      {/* Class */}
      <div className="rounded-xl border border-stone-100 bg-gradient-to-br from-stone-50 to-stone-100/50 p-4">
        <p className="text-xs font-semibold uppercase tracking-wider text-stone-400">Class</p>
        <p className="mt-1 font-semibold text-stone-800">{classInfo.name}</p>
      ...
      {/* Student */}
      <div className="rounded-xl border border-stone-100 bg-gradient-to-br from-stone-50 to-stone-100/50 p-4">
        <p className="text-xs font-semibold uppercase tracking-wider text-stone-400">Student</p>
        ...
        {values.student.gender && (
          <p className="text-sm text-stone-600">Gender: {values.student.gender}</p>
        )}
        {values.student.medicalNotes && (
          <p className="text-sm text-stone-600">Medical: {values.student.medicalNotes}</p>
        )}
      </div>

      {/* Parent */}
      <div className="rounded-xl border border-stone-100 bg-gradient-to-br from-stone-50 to-stone-100/50 p-4">
        <p className="text-xs font-semibold uppercase tracking-wider text-stone-400">Parent / Guardian</p>
        <p className="mt-1 font-semibold text-stone-800">
          {values.parent.firstName} {values.parent.lastName}
        </p>
        <p className="text-sm text-stone-600">{values.parent.email}</p>
        {values.parent.phone && (
          <p className="text-sm text-stone-600">{values.parent.phone}</p>
        )}
      ...
      {/* Promo Code */}
      <div className="rounded-xl border border-stone-100 bg-gradient-to-br from-stone-50 to-stone-100/50 p-4">
        <p className="text-xs font-semibold uppercase tracking-wider text-stone-400 mb-2">Promo Code</p>
        {appliedPromo ? (
          ...
          <div>
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 rounded-lg border border-stone-200 bg-white px-3 py-2 text-sm font-mono transition-shadow input-glow"
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    e.preventDefault();
                    handleValidatePromo();
              ...
              <button
                ...
                className="inline-flex h-10 items-center gap-1.5 rounded-lg bg-stone-100 px-4 text-xs font-medium text-stone-700 transition-colors hover:bg-stone-200 disabled:opacity-50"
              >
