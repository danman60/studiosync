...
function InvoiceList({ onView }: { onView: (id: string) => void }) {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="font-display text-[clamp(1.5rem,2.5vw,2rem)] italic text-stone-800">Invoices & Auto-Pay</h1>
        <p className="mt-1 text-sm text-stone-500">View invoices and manage your auto-pay plans.</p>
      </div>

      {/* Active Auto-Pay Plans */}
      {activePlans.length > 0 && (
        <div className="mb-8">
          <h2 className="text-sm font-semibold uppercase tracking-wider text-stone-400 mb-3">Auto-Pay Plans</h2>
      ...
      {!invoices.isLoading && (invoices.data?.length ?? 0) === 0 && activePlans.length === 0 && (
        <div className="empty-state">
          ...
          <p className="text-sm font-medium text-stone-600">No invoices yet</p>
          <p className="mt-1 text-xs text-stone-400">Invoices from your studio will appear here.</p>
        </div>
      )}
      ...
      {unpaid.length > 0 && (
        <div className="mb-6">
          <h2 className="text-sm font-semibold uppercase tracking-wider text-stone-400 mb-3">Due</h2>
          <div className="space-y-3">
            {unpaid.map((inv) => (
              <button
                key={inv.id}
              ...
              >
                <div>
                  <p className="font-medium text-stone-800">{inv.invoice_number}</p>
                  <p className="text-sm text-stone-500">Due: {inv.due_date}</p>
                </div>
                <div className="text-right">
                  <p className="text-lg font-bold text-stone-800">{formatCents(inv.total - inv.amount_paid)}</p>
      ...
      {paid.length > 0 && (
        <div>
          <h2 className="text-sm font-semibold uppercase tracking-wider text-stone-400 mb-3">Paid</h2>
          <div className="space-y-3">
            {paid.map((inv) => (
              <button
                key={inv.id}
              ...
              >
                <div>
                  <p className="font-medium text-stone-800">{inv.invoice_number}</p>
                  <p className="text-sm text-stone-500">Paid: {inv.paid_at ? new Date(inv.paid_at).toLocaleDateString() : inv.due_date}</p>
                </div>
                <div className="text-right">
                  <p className="text-lg font-bold text-stone-800">{formatCents(inv.total)}</p>
  ...
  return (
    <div className="glass-card rounded-2xl p-5">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="text-right">
          <p className="text-lg font-bold text-stone-800">{formatCents(plan.amount)}</p>
          <p className="text-xs text-stone-500">/{plan.interval}</p>
    ...
    <div>
      ...
      <div className="glass-card-static rounded-2xl p-6 mb-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-stone-500">Issued: {inv.issue_date} | Due: {inv.due_date}</p>
          </div>
          <div className="text-right">
          ...
          <tbody className="divide-y divide-stone-50">
            {items.map((item) => (
              <tr key={item.id} className="table-row-hover">
                ...
                <td className="table-cell text-center text-stone-600">{item.quantity}</td>
                <td className="table-cell text-center text-stone-600">{formatCents(item.unit_price)}</td>
                <td className="table-cell text-right font-medium text-stone-800">{formatCents(item.total)}</td>
              </tr>
            ))}
            <tr className="bg-stone-50/40">
              <td colSpan={3} className="px-5 py-2 text-right text-xs font-medium text-stone-500">Subtotal</td>
              <td className="px-5 py-2 text-right text-sm font-medium text-stone-800">{formatCents(inv.subtotal)}</td>
            </tr>
            {inv.tax_rate > 0 && (
              <tr className="bg-stone-50/40">
                <td colSpan={3} className="px-5 py-2 text-right text-xs font-medium text-stone-500">Tax</td>
                <td className="px-5 py-2 text-right text-sm font-medium text-stone-800">{formatCents(inv.tax_amount)}</td>
              </tr>
            )}
            <tr className="bg-stone-50/60">
              <td colSpan={3} className="px-5 py-3 text-right text-sm font-semibold text-stone-800">
                {canPay ? 'Amount Due' : 'Total'}
              </td>
              <td className="px-5 py-3 text-right text-sm font-bold text-stone-800">
        ...
        <div className="glass-card-static rounded-2xl p-6 text-center">
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            ...
            <button
              ...
              disabled={paymentStatus === 'loading' || createPI.isPending}
              className="inline-flex h-12 items-center gap-2 rounded-xl border border-stone-200 bg-white px-6 text-sm font-medium text-stone-700 hover:bg-stone-50 disabled:opacity-50 transition-colors"
            >
              <Building2 size={18} />
              Pay via Bank (ACH)
            </button>
          ...
          <p className="mt-2 text-xs text-stone-400">Secure payment powered by Stripe</p>
        </div>
      )}
      ...
      {inv.notes && (
        <div className="mt-4 glass-card-static rounded-2xl p-5">
          <h3 className="text-sm font-semibold text-stone-800 mb-2">Notes</h3>
