...
const CATEGORY_BADGE: Record<string, string> = {
  general: 'bg-stone-500/15 text-stone-600',
  ...
  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="font-display text-[clamp(1.5rem,2.5vw,2rem)] italic text-stone-800">Message Templates</h1>
          <p className="mt-1 text-sm text-stone-500">Reusable templates with merge fields for messaging.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        {templates.data?.length === 0 && (
          <div className="col-span-full empty-state">
            ...
            <p className="mt-3 text-sm text-stone-400">No templates yet. Create your first one.</p>
          </div>
        )}
          ...
          <div key={t.id} className={`glass-card rounded-2xl p-5 animate-fade-in-up stagger-${Math.min(idx + 1, 8)} ${!t.is_active ? 'opacity-50' : ''}`}>
            <div className="flex items-start justify-between mb-2">
              <div>
                ...
                {t.subject && <p className="text-xs text-stone-500 mt-0.5">Subject: {t.subject}</p>}
              </div>
              <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${CATEGORY_BADGE[t.category] ?? ''}`}>
                {t.category}
              </span>
            </div>
            <p className="text-xs text-stone-500 line-clamp-3 mb-3">{t.body}</p>
            {t.merge_fields.length > 0 && (
              <div className="flex flex-wrap gap-1 mb-3">
                {t.merge_fields.map((f: string) => (
                  <span key={f} className="rounded bg-primary-50 px-1.5 py-0.5 text-[10px] text-primary font-mono">{f}</span>
                ))}
              </div>
            )}
            <div className="flex items-center gap-1 border-t border-stone-100 pt-3">
              <button onClick={() => setPreviewId(t.id)} className="icon-btn text-stone-400 hover:text-primary" title="Preview">
                <Eye size={14} />
              </button>
              <button onClick={() => setEditId(t.id)} className="icon-btn text-stone-400 hover:text-primary" title="Edit">
                <Pencil size={14} />
              </button>
              <button onClick={() => toggleMut.mutate({ id: t.id, is_active: !t.is_active })} className="icon-btn text-stone-400 hover:text-amber-600" title={t.is_active ? 'Deactivate' : 'Activate'}>
                {t.is_active ? '~' : '+'}
              </button>
              <button
              ...
              <button
                ...
                className="icon-btn text-stone-400 hover:text-stone-600 ml-auto"
                title="Copy body"
              >
                <Copy size={14} />
              </button>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="h-11 w-full rounded-xl border border-stone-200 bg-white px-3 text-sm input-glow" required />
          </div>
          <div>
            <label className="mb-1 block text-xs font-medium text-stone-600">Category</label>
            <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="h-11 w-full rounded-xl border border-stone-200 bg-white px-3 text-sm input-glow">
              {CATEGORIES.map((c) => <option key={c} value={c}>{c.charAt(0).toUpperCase() + c.slice(1)}</option>)}
            </select>
          </div>
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium text-stone-600">Subject (optional)</label>
          <input value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} className="h-11 w-full rounded-xl border border-stone-200 bg-white px-3 text-sm input-glow" placeholder="e.g., Invoice Ready: {amount}" />
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium text-stone-600">Body</label>
          <textarea value={form.body} onChange={(e) => setForm({ ...form, body: e.target.value })} rows={5} className="w-full rounded-xl border border-stone-200 bg-white px-3 py-2 text-sm input-glow font-mono" required />
        </div>

        {/* Merge field buttons */}
        <div>
          <label className="mb-1 block text-xs font-medium text-stone-600">Insert Merge Field</label>
          <div className="flex flex-wrap gap-1">
            {MERGE_OPTIONS.map((f) => (
              <button
                ...
                className={`rounded-lg px-2 py-1 text-xs font-mono transition-colors ${
      ...
      <div className="space-y-3">
        {preview.data?.subject && (
          <div>
            ...
            <p className="text-sm font-medium text-stone-800">{preview.data.subject}</p>
          </div>
        )}
        <div>
          <label className="text-xs font-medium text-stone-500">Body</label>
          <div className="mt-1 rounded-xl bg-stone-50 p-4 text-sm text-stone-700 whitespace-pre-wrap">
            {preview.data?.body ?? 'Loading...'}
          </div>
        </div>
        <p className="text-xs text-stone-400 italic">Merge fields shown with sample data.</p>
      </div>
    </Modal>
  );
}
