...
export default function EventsPage() {
  ...
  if (view === 'orders' && selectedEventId) {
    ...
    return (
      <div>
        <button onClick={() => { setView('list'); setSelectedEventId(null); }} className="mb-4 inline-flex items-center gap-1 text-sm text-stone-500 hover:text-primary transition-colors">
          <ChevronLeft size={16} /> Back to Events
        </button>
        <div className="mb-6">
          <h1 className="font-display text-[clamp(1.5rem,2.5vw,2rem)] italic text-stone-800">Ticket Orders</h1>
          <p className="mt-1 text-sm text-stone-500">{event?.name ?? 'Event'}</p>
        </div>
            ...
            <tbody className="divide-y divide-stone-50">
              {(orders ?? []).map((o) => (
                <tr key={o.id} className="table-row-hover">
                  <td className="table-cell font-medium text-stone-800">{o.buyer_name}</td>
                  <td className="table-cell text-stone-500">{o.buyer_email}</td>
                  <td className="table-cell text-center text-stone-700">{o.quantity}</td>
                  <td className="table-cell text-center text-stone-700">{formatCents(o.total_amount)}</td>
                  <td className="table-cell text-center">
                    <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_BADGE[o.status] ?? STATUS_BADGE.draft}`}>
                      {o.status}
                    </span>
                  </td>
                  <td className="table-cell text-xs text-stone-400">{new Date(o.created_at).toLocaleDateString()}</td>
                </tr>
              ))}
              {(orders ?? []).length === 0 && (
                <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-stone-400">No orders yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
  ...
  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="font-display text-[clamp(1.5rem,2.5vw,2rem)] italic text-stone-800">Events</h1>
          <p className="mt-1 text-sm text-stone-500">Manage performances, recitals, and showcases.</p>
        </div>
          ...
          {(events ?? []).map((evt, idx) => (
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <span className={`shrink-0 rounded-full px-2.5 py-0.5 text-xs font-medium ${STATUS_BADGE[evt.status]}`}>
                  {evt.status}
                </span>
              </div>
              ...
              <div className="mt-4 flex items-center gap-2 border-t border-stone-100 pt-3">
                ...
                <button
                  ...
                  className="inline-flex h-10 items-center gap-1.5 rounded-xl bg-stone-100 px-3 text-xs font-medium text-stone-600 transition-colors hover:bg-stone-200"
                >
      ...
      {!isLoading && (events ?? []).length === 0 && (
        <div className="empty-state">
          <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-primary-50">
            <Ticket size={24} className="text-primary-light" />
          </div>
          <p className="text-sm font-medium text-stone-600">No events yet</p>
          <p className="mt-1 text-xs text-stone-400">Create your first performance or recital.</p>
        </div>
      )}
...
function CreateEventModal({
  ...
  return (
    <Modal open={true} onClose={onClose} title="New Event">
      ...
      >
        <div>
          <label className="mb-1 block text-xs font-medium text-stone-600">Event Name *</label>
          <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required className="h-11 w-full rounded-xl border border-stone-200 bg-white px-3 text-sm input-glow" />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="mb-1 block text-xs font-medium text-stone-600">Date *</label>
            <input type="date" value={form.event_date} onChange={(e) => setForm({ ...form, event_date: e.target.value })} required className="h-11 w-full rounded-xl border border-stone-200 bg-white px-3 text-sm input-glow" />
          </div>
          <div>
            <label className="mb-1 block text-xs font-medium text-stone-600">Time</label>
            <input type="time" value={form.event_time} onChange={(e) => setForm({ ...form, event_time: e.target.value })} className="h-11 w-full rounded-xl border border-stone-200 bg-white px-3 text-sm input-glow" />
          </div>
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium text-stone-600">Location</label>
          <input value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} className="h-11 w-full rounded-xl border border-stone-200 bg-white px-3 text-sm input-glow" />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="mb-1 block text-xs font-medium text-stone-600">Ticket Price ($)</label>
            <input type="number" step="0.01" min="0" value={form.ticket_price} onChange={(e) => setForm({ ...form, ticket_price: e.target.value })} className="h-11 w-full rounded-xl border border-stone-200 bg-white px-3 text-sm input-glow" />
          </div>
          <div>
            <label className="mb-1 block text-xs font-medium text-stone-600">Max Tickets</label>
            <input type="number" min="1" value={form.max_tickets} onChange={(e) => setForm({ ...form, max_tickets: e.target.value })} placeholder="Unlimited" className="h-11 w-full rounded-xl border border-stone-200 bg-white px-3 text-sm input-glow" />
          </div>
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium text-stone-600">Description</label>
          <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={3} className="w-full rounded-xl border border-stone-200 bg-white px-3 py-2.5 text-sm input-glow" />
        </div>
