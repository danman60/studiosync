  ...
  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="font-display text-[clamp(1.5rem,2.5vw,2rem)] italic text-stone-800">Report Cards</h1>
          <p className="mt-1 text-sm text-stone-500">Track progress mark completion across classes.</p>
        </div>
        <select
          value={period}
          onChange={(e) => setPeriod(e.target.value)}
          className="h-11 rounded-xl border border-stone-200 bg-white px-4 text-sm text-stone-700 transition-shadow input-glow"
        >
          {PERIODS.map((p) => (
        ...
        <div className="glass-card rounded-2xl bg-primary-50 p-6 animate-fade-in-up stagger-1">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-stone-500">Active Classes</p>
            </div>
          </div>
        </div>
        <div className="glass-card rounded-2xl bg-gradient-to-br from-emerald-500/10 to-emerald-600/5 p-6 animate-fade-in-up stagger-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-stone-500">Classes with Marks</p>
            </div>
          </div>
        </div>
        <div className="glass-card rounded-2xl bg-gradient-to-br from-blue-500/10 to-blue-600/5 p-6 animate-fade-in-up stagger-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-stone-500">Enrolled Students</p>
            </div>
          </div>
        </div>
        <div className="glass-card rounded-2xl bg-primary-50 p-6 animate-fade-in-up stagger-4">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary-100 text-primary">
              <FileText size={20} />
            </div>
            <div>
              ...
              <p className="text-xs text-stone-500">Students Graded</p>
            </div>
          </div>
        </div>
      </div>
              ...
              {overview.data?.length === 0 && (
                <tr>
                  <td colSpan={7} className="table-cell text-center py-14">
                    ...
                    <p className="mt-3 text-sm text-stone-400">No active classes found.</p>
                  </td>
                </tr>
              )}
              {overview.data?.map((cls) => (
                <tr key={cls.id} className="table-row-hover">
                  ...
                  <td className="table-cell text-stone-600">
                    ...
                    {cls.level && (
                      <span className="ml-1 inline-block rounded-full bg-stone-100 px-2 py-0.5 text-xs text-stone-600">
                        {cls.level}
                      </span>
                    )}
                  </td>
                  <td className="table-cell text-stone-600">{cls.instructor ?? '—'}</td>
                  <td className="table-cell text-stone-600">{cls.enrolledStudents}</td>
                  <td className="table-cell">
                    <span className={cls.markedStudents > 0 ? 'text-emerald-600 font-medium' : 'text-stone-400'}>
                      {cls.markedStudents}
                    </span>
                  </td>
                  <td className="table-cell">
                      ...
                      <div className="h-1.5 w-20 rounded-full bg-stone-100 overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all ${
                            ...
                            cls.completionPercent > 0 ? 'bg-primary' : 'bg-stone-200'
                          }`}
                          style={{ width: `${cls.completionPercent}%` }}
                        />
                      </div>
                      <span className="text-xs text-stone-500 w-8">{cls.completionPercent}%</span>
                    </div>
                  </td>
                  <td className="table-cell">
